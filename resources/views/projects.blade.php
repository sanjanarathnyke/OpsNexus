            ...
            <main class="w-full grow p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 border-b border-gray-200 pb-8">
                    <div class="bg-white overflow-hidden rounded-xl shadow-sm border border-gray-100 p-5">
                        <div class="flex items-center">
                            <div class="flex-shrink-0 bg-orange-50 rounded-md p-3 text-orange-600 font-bold text-xl">
                                ◫
                            </div>
                ...
                <div class="border-b border-gray-200">
                    <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                        <button class="filter-tab active border-orange-500 text-orange-600 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors" onclick="setTab(this,'all')">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($projects as $project)
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <a href="{{ $project->repo_url }}" target="_blank" class="text-sm border border-gray-200 rounded-md px-2 py-1 flex items-center w-fit text-orange-600 hover:bg-orange-50 hover:border-orange-200 transition-colors">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex -space-x-2 overflow-hidden">
                                                @php
                                                    ...
                                                    $avatarColors = ['bg-orange-500', 'bg-pink-500', 'bg-yellow-500', 'bg-green-500', 'bg-purple-500', 'bg-red-500'];
                                                @endphp
                                                ...
                                                @empty
                                                    <div class="inline-flex h-8 w-8 rounded-full ring-2 ring-white bg-gray-50 items-center justify-center text-white text-xs">—</div>
                                                @endforelse
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end space-x-3">
                                                <button onclick="openModal({{ json_encode($project) }})" class="text-orange-600 hover:text-orange-900 transition-colors">Edit</button>
                                ...
                                @if (count($projects) == 0)
                                    <tr>
                                        <td colspan="7" class="px-6 py-12 text-center text-gray-500 text-sm">
                                            No projects found. <button type="button" onclick="openModal()" class="text-orange-600 font-medium hover:underline cursor-pointer">Click "Add Project" to create one.</button>
                                        </td>
                                    </tr>
                                @endif
                            </tbody>
        ...
        <div class="flex items-center justify-center min-h-screen p-4 text-center sm:p-0">
            ...
            <div class="relative bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-slate-200">
                    ...
                    <form id="projectForm" method="POST" action="{{ route('projects.store') }}">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="project_name" id="project_name" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-orange-500 focus:border-orange-500 sm:text-sm">
                            </div>
                            <div>
                                <label for="description" class="block text-sm font-medium text-gray-700">Description</label>
                                <input type="text" name="description" id="description" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-orange-500 focus:border-orange-500 sm:text-sm">
                            </div>
                            <div>
                                <label for="repo_url" class="block text-sm font-medium text-gray-700">Repo URL</label>
                                <input type="url" name="repo_url" id="repo_url" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-orange-500 focus:border-orange-500 sm:text-sm">
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input type="number" name="progress" id="progress" min="0" max="100" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-orange-500 focus:border-orange-500 sm:text-sm">
                            </div>
                            <div>
                                <label for="payments" class="block text-sm font-medium text-gray-700">Payment</label>
                                <select name="payments" id="payments" required class="mt-1 block w-full bg-white border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-orange-500 focus:border-orange-500 sm:text-sm">
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button type="submit" form="projectForm" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-orange-600 text-base font-medium text-white hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 sm:ml-3 sm:w-auto sm:text-sm">
                        Save Project
                    </button>
