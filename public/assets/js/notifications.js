...
function renderNotificationFeed() {
  ...
  feed.innerHTML = NOTIFICATIONS_DATA.map(n => `
    <div class="group relative p-5 hover:bg-gray-50 transition-colors flex items-start space-x-4 notif-item ${n.unread ? 'bg-blue-50/30' : ''}" data-id="${n.id}">
      ...
      <div class="flex-1 min-w-0">
        <p class="text-sm text-gray-800">
          ${n.text.replace(/<strong>/g, '<span class="font-semibold text-gray-900">').replace(/<\/strong>/g, '</span>').replace(/<span class="repo-name">/g, '<span class="font-semibold text-blue-600">').replace(/<code>/g, '<span class="font-mono text-xs bg-gray-100 px-1.5 py-0.5 rounded text-gray-700">').replace(/<\/code>/g, '</span>')}
        </p>
      ...
      ${n.unread ? `<div class="flex-shrink-0 mt-3 notif-unread-dot">
        <span class="h-2.5 w-2.5 rounded-full bg-blue-600 inline-block shadow-sm ring-2 ring-white"></span>
      </div>` : ''}
    </div>
  `).join('');
